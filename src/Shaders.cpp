...
{
    GLCall(glUniformMatrix4fv(GetUniformLocation(name), 1, GL_FALSE, &matrix[0][0])); // V means passing an float array 
}
...
int Shader::GetUniformLocation(const std::string& name)
{
    ...
    /* That Location Cache is for Performance - glGetUniformLocation will only retrieve first time instead of keep on retrieving when SetUniform4f is called */ 
    GLCall(int location = glGetUniformLocation(m_RendererID, name.c_str())); // when deal with uniform location in OpenGL type is always 32-bit integer
    if (location == -1) // means no location
    {
